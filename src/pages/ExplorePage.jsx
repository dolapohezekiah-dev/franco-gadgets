...
export default function ExplorePage() {
  ...
  const FilterPanel = () => (
    <div className="space-y-6">
      {/* Category */}
      <div>
        <h3 className="text-white font-display text-lg tracking-wider mb-3">CATEGORY</h3>
      ...
      {/* Condition */}
      <div>
        <h3 className="text-white font-display text-lg tracking-wider mb-3">CONDITION</h3>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-mono transition-all border ${
                selectedCondition === c
                  ...
                  : "text-brand-muted border-brand-border hover:border-brand-cyan/30"
              }`}
            >
              {c === "all" ? "All" : c}
            </button>
      ...
      {/* Brands */}
      <div>
        <h3 className="text-white font-display text-lg tracking-wider mb-3">BRAND</h3>
      ...
      {/* Price Range */}
      <div>
        <h3 className="text-white font-display text-lg tracking-wider mb-3">MAX PRICE</h3>
        <input
          type="range"
          min={0}
          max={1000000}
        ...
        <div className="mb-8">
          ...
          {searchQuery && (
            <p className="text-brand-muted font-body mt-2">
              Search results for: <span className="text-brand-light font-semibold">"{searchQuery}"</span>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6 gap-4">
              <p className="text-brand-muted text-sm font-body">
                <span className="text-brand-light font-semibold">{filtered.length}</span> products found
              </p>
              <div className="flex items-center gap-3">
                {/* Mobile filter toggle */}
                <button
                  onClick={() => setShowFilters(!showFilters)}
                  className="lg:hidden flex items-center gap-2 px-3 py-2 bg-brand-card border border-brand-border rounded-xl text-sm text-brand-muted hover:text-brand-cyan font-body transition-colors"
                >
                ...
                <select
                  ...
                  className="bg-brand-card border border-brand-border text-brand-muted rounded-xl px-3 py-2 text-sm font-body focus:outline-none focus:border-brand-cyan transition-colors"
                >
            ...
            {/* Mobile filters */}
            {showFilters && (
              <div className="lg:hidden bg-brand-card border border-brand-border rounded-2xl p-5 mb-6">
                <FilterPanel />
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-20">
                ...
                <h3 className="text-white font-display text-3xl tracking-wider mb-2">NO RESULTS</h3>
                <p className="text-brand-muted font-body">Try adjusting your filters or search term.</p>
              </div>
            ) : (
