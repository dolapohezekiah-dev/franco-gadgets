  ...
  return (
    <div className="min-h-screen bg-brand-dark">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <h1 className="font-display text-5xl text-white tracking-wider mb-10">{title}</h1>
        <div className="prose prose-invert max-w-none font-body text-brand-muted space-y-6">
          {children}
        </div>
      </div>
    </div>
...
export function AboutPage() {
  return (
    <PageWrapper title="ABOUT US" subtitle="Who We Are">
      <div className="bg-brand-card border border-brand-border rounded-2xl p-8 space-y-4 text-brand-muted">
        <p className="text-brand-light text-lg font-body leading-relaxed">
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          {[
            ...
            <div key={stat.l} className="bg-brand-dark rounded-xl p-4 text-center">
              <p className="text-brand-cyan font-display text-4xl">{stat.n}</p>
              <p className="text-brand-muted text-sm mt-1">{stat.l}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <PageWrapper title="FAQS" subtitle="Common Questions">
      <div className="space-y-3">
        {faqs.map((faq) => (
          <div key={faq.q} className="bg-brand-card border border-brand-border rounded-xl p-5">
            <p className="text-white font-semibold font-body mb-2">{faq.q}</p>
    ...
    <PageWrapper title="WARRANTY" subtitle="Our Promise">
      <div className="space-y-4">
        {[
          ...
          <div key={w.title} className="bg-brand-card border border-brand-border rounded-xl p-5">
            <div className="flex justify-between items-start mb-2">
              <p className="text-white font-semibold font-body">{w.title}</p>
              <span className="bg-brand-cyan/10 text-brand-cyan text-xs font-mono px-2 py-1 rounded-full">{w.period}</span>
            </div>
            <p className="text-brand-muted text-sm">{w.details}</p>
          </div>
        ))}
        <div className="bg-brand-card border border-brand-border rounded-xl p-5 mt-4">
          <p className="text-white font-semibold mb-2">How to Claim Warranty</p>
          <p className="text-brand-muted text-sm">Contact us on WhatsApp with your order number and a description of the issue. We'll guide you through the process. Always keep your purchase invoice.</p>
        </div>
      </div>
    </PageWrapper>
  );
    ...
    <PageWrapper title="DELIVERY" subtitle="Shipping Info">
      <div className="space-y-4">
        <div className="bg-brand-card border border-brand-border rounded-xl p-5">
          <p className="text-white font-semibold font-body mb-3">Delivery Zones & Timelines</p>
          <div className="space-y-2">
            {[
              ...
              <div key={row.zone} className="flex items-center justify-between text-sm py-2 border-b border-brand-border last:border-0">
                <div>
                  <p className="text-brand-light">{row.zone}</p>
                  <p className="text-brand-muted text-xs">{row.time}</p>
                </div>
        ...
        <div className="bg-brand-card border border-brand-border rounded-xl p-5">
          <p className="text-white font-semibold mb-2">Important Notes</p>
    ...
    <PageWrapper title="CONTACT US" subtitle="Get In Touch">
      <div className="space-y-4">
        {[
          ...
          <div key={c.title} className="bg-brand-card border border-brand-border rounded-xl p-5 flex items-start gap-4">
            <span className="text-3xl">{c.icon}</span>
            <div>
              <p className="text-brand-muted text-xs font-mono mb-1">{c.title}</p>
              {c.link ? (
                <a href={c.link} target="_blank" rel="noopener noreferrer" className="text-brand-cyan font-body font-semibold hover:underline">
                  {c.info}
                </a>
              ) : (
                <p className="text-brand-light font-body font-semibold">{c.info}</p>
              )}
              <p className="text-brand-muted text-xs mt-0.5">{c.sub}</p>
            </div>
          </div>
        ))}
        <div className="bg-brand-card border border-brand-cyan/20 rounded-xl p-5 text-center">
          <p className="text-white font-body mb-3">Ready to order? Chat with us directly!</p>
          <a
            href="https://wa.me/2348012345678"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white px-6 py-3 rounded-xl font-display text-lg tracking-wider transition-all"
          >
